...
RF24 radio(9, 10); // select  CE,CSN  pins
...
void loop()
{
  ...
  data.throttle = map( analogRead(A0), 13, 524, 1015, true );
  data.yaw      = map( analogRead(A1),  1, 505, 1020, true );
  data.pitch    = map( analogRead(A2), 12, 544, 1021, true );
  data.roll     = map( analogRead(A3), 34, 522, 1020, true );
